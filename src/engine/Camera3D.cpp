...
Camera3D::Camera3D(glm::vec3 position, float32_t yaw, float32_t pitch)
    : m_position(position),
      m_yaw(yaw),
      m_pitch(pitch)
{
...
void Camera3D::processMouseDelta(float32_t deltaX, float32_t deltaY) noexcept
{
...
glm::mat4 Camera3D::getProjectionMatrix(float32_t aspect) const noexcept
{
...
{
    float32_t aspect = viewport.y > 0.f ? (viewport.x / viewport.y) : 1.f;
...
bool Camera3D::isSphereInFront(const glm::vec3& position, float32_t radius) const noexcept
{
    float32_t depth = glm::dot(position - m_position, m_forward);
...
Ray Camera3D::screenPointToRay(double x, double y, const glm::vec2& viewport) const noexcept
{
    ...
    float32_t ndcX = (2.f * static_cast<float32_t>(x) / viewport.x) - 1.f;
    float32_t ndcY = 1.f - (2.f * static_cast<float32_t>(y) / viewport.y);
...
void Camera3D::updateVectors() noexcept
{
    ...
    float32_t yawRad = glm::radians(m_yaw + 90.f);
    float32_t pitchRad = glm::radians(m_pitch);
