...
void Camera2D::setZoom(float32_t factor) noexcept
{
    if (factor > 0.0f)
    {
...
glm::vec3 Camera2D::worldToNDC(const glm::vec3& position) const noexcept
{
    ...
    float32_t x = (relative.x / (size.x * 0.5f)) * m_zoom;
    float32_t y = (relative.y / (size.y * 0.5f)) * m_zoom;
...
glm::vec3 Camera2D::ndcToWorld(const glm::vec3& ndc) const noexcept
{
    ...
    float32_t halfW = size.x * 0.5f;
    float32_t halfH = size.y * 0.5f;
...
void Camera2D::clampOffset()
{
    ...
    float32_t visHalfW = halfW / m_zoom;
    float32_t visHalfH = halfH / m_zoom;

    float32_t maxOffX = std::max(0.f, halfW - visHalfW);
    float32_t maxOffY = std::max(0.f, halfH - visHalfH);
